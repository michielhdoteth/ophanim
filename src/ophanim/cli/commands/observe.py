...
from ophanim.core.video import probe, extract_frames, estimate_processing_cost, auto_fps, _downscale
...
from ophanim.providers.base import VlmResponse, TokenUsage
...
def observe_cmd(
    ...
    from ophanim.core.download import is_url
    ...
    # Download from URL if remote
    if is_remote:
        from ophanim.core.download import download_video
        console.print("[dim]Downloading from URL...[/dim]")
        try:
            dl_result = download_video(path, max_height=720)
            input_path = Path(dl_result["path"])
            console.print(f"[dim]Downloaded: {dl_result['title']} ({dl_result['duration']:.0f}s)[/dim]")
    ...
    focus_end = None
    if start_time:
        from ophanim.core.video import parse_time
    ...
    if is_image:
        _handle_image(input_path, question, prompt, json_output, config, actual_resolution, max_tokens)
    else:
        _handle_video(input_path, question, prompt, json_output, save_memory, transcribe_audio, diarize_audio, force,
                      ...
                      focus_start, focus_end, cue_timestamps)
...
def _handle_video(path: Path, question: Optional[str], custom_prompt: Optional[str],
    ...
    if has_focus:
        from ophanim.core.video import auto_fps_focus
        ...
        from ophanim.core.video import extract_keyframes_ffmpeg, dedupe_frames_ffmpeg
        ...
        from ophanim.core.video import extract_scene_frames_ffmpeg, dedupe_frames_ffmpeg
    ...
    if detail == "transcript" and not frames:
        ...
        if diarize_audio and transcript and transcript.segments:
            try:
                from ophanim.providers.diarizer import DiarizerProvider, merge_transcript_with_diarization
                from ophanim.core.audio import extract_audio
    ...
    pinned_frames = []
    if cue_timestamps:
        from ophanim.core.video import extract_at_timestamps
