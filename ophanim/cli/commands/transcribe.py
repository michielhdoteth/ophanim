...
def transcribe_cmd(
    ...
    if is_remote:
        ...
        _safe_print("[dim]Downloading from URL...[/dim]")
        try:
            dl_result = download_video(path, audio_only=False, write_subs=True)
            input_path = Path(dl_result["path"])
            _safe_print(f"[dim]Downloaded: {dl_result['title']}[/dim]")
    ...
    if is_remote and dl_result.get("subs_file") and os.path.exists(dl_result["subs_file"]):
        try:
            from ophanim.core.captions import parse_vtt, filter_range
            sub_content = Path(dl_result["subs_file"]).read_text(encoding="utf-8", errors="replace")
            caps = parse_vtt(sub_content)
            if caps:
                from ophanim.providers.whisper import Transcript, TranscriptSegment
