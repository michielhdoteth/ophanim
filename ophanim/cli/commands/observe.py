...
def observe_cmd(
    ...
    if is_image:
        _handle_image(input_path, question, json_output, config, actual_resolution, max_tokens)
    else:
        _handle_video(input_path, question, json_output, save_memory, transcribe_audio, force,
                      config, mode, actual_fps, actual_max_frames, actual_resolution, max_tokens)


def _handle_image(path: Path, question: Optional[str], json_output: bool,
                  config: dict, resolution: int, override_max_tokens: Optional[int] = None):
    ...
    try:
        response = provider.describe_image(image, question)
